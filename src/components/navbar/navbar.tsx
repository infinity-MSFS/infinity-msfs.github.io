...
export const Navbar: FC<T_navbarProps> = (
	props: T_navbarProps,
): JSX.Element => {
	...
	const handleClickOutside = (event: MouseEvent): void => {
		if (
			...
			(hamburgerButtonRef.current === null ||
	...
	const getButtons = (): JSX.Element[] => {
		const buttons = props.buttons.map(
			(button: T_navbarButtonProps, index: number) => {
	...
	const hamburgerMenu = (): JSX.Element => {
		...
		return (
			...
			>
				{buttons}
				<div className="mt-4 w-full px-8">{getAuthButton()}</div>
			</div>
		);
	};
